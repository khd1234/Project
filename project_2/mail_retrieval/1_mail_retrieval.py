...
firsturl = "http://mbox.dr-chuck.net/sakai.devel/"
...
while True:
    ...
    start = start + 1
    cur.execute('SELECT id FROM Messages WHERE id=?', (start,))  # Checks if the start id already in the table
    try:
        row = cur.fetchone()
        if row is not None:  # Loop out if already in the table
            continue
    except:
        row = None

    many = many - 1
    url = firsturl + str(start) + '/' + str(start + 1)
    ...
    if len(y) == 1:
        ...
        tdate = tdate[:26]
        try:
            sent_at = DateFormatChange(tdate)
        except:
            print(text)
    ...
    # Extracting subject from header
    subject = None
    z = re.findall('\nSubject: (.*)\n', hdr)
    if len(z) == 1:
        subject = z[0].strip().lower()

    # Reset the fail counter and inserting into database
    fail = 0
...
conn.commit()
cur.close()
print('Run "2_clean_maildata" to clean the raw data')
