...
for message_row in cur:
    senders_dict[message_row[0]] = message_row[1]
...
print("Loaded messages=", len(messages), "senders=", len(senders_dict))

send_orgs_dict = dict()
for (message_id, message) in list(messages.items()):
    sender = message[1]
    pieces = senders_dict[sender].split("@")
    if len(pieces) != 2:
        continue
    dns = pieces[1]
    send_orgs_dict[dns] = send_orgs_dict.get(dns, 0) + 1

# pick the top schools
orgs = sorted(send_orgs_dict, key=send_orgs_dict.get, reverse=True)
orgs = orgs[:10]
